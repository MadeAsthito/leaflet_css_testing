...
initMarker = () => {
    ...
    var id_restaurant = urlParams.get(id_restaurant)
    ...
    axios
    .post(api_url + "/restaurants/" + id_restaurant, {
        headers,
    })
    .then((response) => {
        ...
        document.getElementById('descriptionInput').value = response.data.description;
        document.getElementById('emailInput').value = response.data.email;
        document.getElementById('phoneNumberInput').value = response.data.phone;

        newMarker = addMarker([response.latitude, response.longitude]);
    })
    .catch((error) => {
        // Handle error
